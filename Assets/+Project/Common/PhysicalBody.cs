    ...
    public class PhysicalBody : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            if (collisionShape == CollisionShape.BOX)
            {
                ...
                size = transform.rotation * new Vector3(size.x * transform.lossyScale.x,
                    size.y * transform.lossyScale.y, size.z * transform.lossyScale.z);
                Gizmos.color = Color.red;
                Vector3 topLeft = new Vector3(transform.position.x - size.x / 2.0f,
                    transform.position.y - size.y / 2.0f, transform.position.z - size.z / 2.0f);
                Vector3 bottomRight = new Vector3(transform.position.x + size.x / 2.0f,
                    transform.position.y + size.y / 2.0f, transform.position.z + size.z / 2.0f);
        ...
        private bool CheckGround()
        {
            ...
            switch (collisionShape)
            {
                case CollisionShape.SPHERE:
                    ...
                    colliders = Physics.OverlapSphere(rigidbody.position + Vector3.down * groundedThreshold,
                        radius * 0.9f, groundLayers);
                ...
                case CollisionShape.BOX:
                    Vector3 size = (collider as BoxCollider).size;
                    size = new Vector3(size.x * transform.lossyScale.x, size.y * transform.lossyScale.y,
                        size.z * transform.lossyScale.z);
                    colliders = Physics.OverlapBox(rigidbody.position + Vector3.down * groundedThreshold, size * 0.5f,
                        transform.rotation, groundLayers);
                ...
                case CollisionShape.CAPSULE:
                    ...
                    float capsuleHeight =
                        ...
                        (collider as CapsuleCollider).height; //we halve it because we don't want the nubs
                    Vector3 direction = GetCapsuleDirectionVector(collider as CapsuleCollider);
                    colliders = Physics.OverlapCapsule(
                        rigidbody.position + direction * (capsuleHeight * 0.5f - capsuleRadius),
                        rigidbody.position - direction * (capsuleHeight * 0.5f - capsuleRadius) +
                        Vector3.down * groundedThreshold, capsuleRadius, groundLayers);
        ...
        {
            if (Raycast(desiredDirection, surfaceThreshold, surfaceLayers, out normal, out surfaceDistance,
                    out surface))
                return true;

            if (Raycast(-rigidbody.transform.up, surfaceThreshold, surfaceLayers, out normal, out surfaceDistance,
                    out surface))
                return true;

            if (lastSurfaceNormal.magnitude > 0.0f && Raycast(-lastSurfaceNormal, surfaceThreshold, surfaceLayers,
                    out normal, out surfaceDistance, out surface))
                return true;
        ...
        /// </summary>
        private bool Raycast(Vector3 direction, float distance, LayerMask layers, out Vector3 normal,
            out float surfaceDistance, out Collider surface)
        {
            //calculate normal
