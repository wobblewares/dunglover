...
namespace Wobblewares.Prototyping
{
	...
	public class ControllableActor : MonoBehaviour, IControllable
	{
		// Physics based on Very Very Valet implementation.
		// https://www.youtube.com/watch?v=qdskE8PJy6Q
		#region Inspector Variables

		[Header("Movement")] [SerializeField] private float maxSpeed = 80.0f;
		...
		[Header("Jumping")] [SerializeField] private float jumpForce = 10.0f;
		...
		private void FixedUpdate()
		{
			...
			if (physicalBody.IsOnSurface)
				rigidbody.AddForce(physicalBody.SurfaceNormal * Physics.gravity.y * rigidbody.mass,
		...
		private void UpdateMovement()
		{
			...
			_targetVelocity =
				Vector3.MoveTowards(_targetVelocity, goalVelocity, scaledAcceleration * Time.fixedDeltaTime);
		...
		private void StandUpright()
		{
			...
			rigidbody.AddTorque((rotationAxis * (rotRadians * uprightJointSpringStrength)) -
			                    (rigidbody.angularVelocity * uprightJointSpringDamper));
		}
